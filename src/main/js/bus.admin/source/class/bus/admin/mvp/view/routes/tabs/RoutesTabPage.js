qx.Class.define("bus.admin.mvp.view.routes.tabs.RoutesTabPage", {
			...
			members : {
				...
				initWidgets : function() {
					...
					// this.__mainContainer.setBackgroundColor("blue");
					...
					this.__filterField.addListener("input",
				...
				refreshWidgets : function() {
					var transportType = this.__routesLeftPanel
							.getTransportType();
				...
				__createRouteTabView : function() {
					...
					radioDirect.addListener("execute", this.on_radio_direct,
							this);
					radioReverse.addListener("execute", this.on_radio_reverse,
							this);
				...
				on_resize_tabView : function(e) {
					if (this.__stationsTable != null) {
						this.__stationsTable.setWidth(this.__routeTabView
								.getBounds().width
								- this.__stationsTable.getBounds().left - 130);
						this.__stationsTable.setHeight(this.__routeTabView
								.getBounds().height
					...
					if (this.__diRadioGroup != null) {
						this.__diRadioGroup.setUserBounds(this.__routeTabView
										.getBounds().width
										- 120, 5, this.__diRadioGroup
										.getBounds().width, this.__diRadioGroup
										.getBounds().height);
					}
					this.btn_delete_station.setUserBounds(this.__routeTabView
									.getBounds().width
									- 120, 80, this.btn_delete_station
									.getBounds().width, this.btn_delete_station
									.getBounds().height);
				...
				on_resize_mainContainer : function(e) {
					...
					if (this.__routesTable != null) {
						this.__routesTable.setWidth(this.__mainContainer
								.getBounds().width
								- this.__routesTable.getBounds().left - 115);
						this.btn_timeTable.setUserBounds(this.__mainContainer
										.getBounds().width
										- 110, 10, this.btn_timeTable
										.getBounds().width, this.btn_timeTable
										.getBounds().height);
						this.btn_edit.setUserBounds(this.__mainContainer
										.getBounds().width
										- 110, 45,
								this.btn_edit.getBounds().width, this.btn_edit
										.getBounds().height);
						this.btn_change.setUserBounds(this.__mainContainer
										.getBounds().width
										- 110, 80,
								this.btn_change.getBounds().width,
								this.btn_change.getBounds().height);
						this.btn_delete.setUserBounds(this.__mainContainer
										.getBounds().width
										- 110, 115,
								this.btn_delete.getBounds().width,
								this.btn_delete.getBounds().height);

					}
					if (this.__routeTabView != null) {
						this.__routeTabView.setWidth(this.__mainContainer
								.getBounds().width
								- this.__routeTabView.getBounds().left - 10);
						this.__routeTabView.setHeight(this.__mainContainer
								.getBounds().height
